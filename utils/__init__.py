...
def get_max_memory(device:List[str]):
    max_memory = {"cpu": "0GiB"}
    for i in range(8):
        max_memory[i] = "32GiB" if f"cuda:{i}" in device else "0GiB"
    return max_memory
...
def load_vllm_for_edit(model_name:str, device:List[str])->BaseVLLMForEdit:
    ...
    if 'llava' in model_name:
        ...
        return LlavaForEdit(model_path, device, max_memory, True)
    elif 'blip2' in model_name:
        from editor.vllms_for_edit import BLIP2OPTForEdit
        return BLIP2OPTForEdit(model_path, device, max_memory)
...
def load_vllm_editor(editor_name:str, edit_model_name:str, device:List[str], 
        extra_devices:List[str], editor_ckpt_path = None, for_train = False):
    ...
    if editor_name == 'vead':
        ...
        data_proc_device = extra_devices if for_train else None

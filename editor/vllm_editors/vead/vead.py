...
class VEAD(VLLMBaseEditorWithTraining):
    ...
    def get_edit_signal_for_one_request(self, prompt:str, image:PILImage, target_new:str):
        ...
        # prompt_end is length of input_embeds minus length of label_ids (the length of the image tokens plus the question tokens)
    ...
    def preprocess_train_data(self, raw_data: BaseVLLMEditData, start_i = 0, end_i = None) -> List:
        ...
        for i in tqdm(range(start_i, end_i), 'Pre-processing train data'):
            ...
            # ==========
            # So the evaluation are still based on teacher-forcing?
            # ==========
            if not os.path.exists(save_path):
                # Reliability
                ...
                (input_embeds, vt_range), label_ids, label_masks = self.vllm.prompts_imgs_target_to_xym(prompt, img, target) # teacher-forcing
    ...
    def __get_xy_for_influence_mapper__(self, rel_xym, gen_xym, loc_xym
        ...
        sps = [rel_xym, *[v for v in gen_xym.values()]] # Here include both text- and image-generality
        ...
        l = l[:vtsn] # Randomly sampled vision token ids
    ...
    def organize_batch_data(self, a_batch_of_training_data_paths:List):
        ...
        gen_xym = {} # keys: text_rephrase, image_rephrase
        loc_xym = {} # keys: image_loc
